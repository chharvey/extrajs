...
export default Promise.all([
	...
	test((() => { try { return `${xjs.Math.interpolateArithmetic(NaN     , 0)}` } catch (e) { return e.name } })(), 'RangeError'   ),
	...
	test((() => { try { return `${xjs.Math.interpolateGeometric(NaN     , 0)}` } catch (e) { return e.name } })(), 'RangeError'   ),
])
